...
/* ═══════════════════════════════════════════════════════════════════════════════
 *  ثوابت المعالج (delegated to unified CPU core)
 * ═══════════════════════════════════════════════════════════════════════════════ */

constexpr int MEM_SIZE   = DHAD_MEM_SIZE;
constexpr int PROG_START = DHAD_PROG_START;
constexpr int STACK_SIZE = DHAD_STACK_SIZE;
...
/* ═══════════════════════════════════════════════════════════════════════════════
 *  GUI-specific CPU wrapper (adds string output + inst_count + stack_top)
 * ═══════════════════════════════════════════════════════════════════════════════ */

static string gui_output_buf;

static void gui_output_callback(int value, bool is_char, void *userdata) {
    (void)userdata;
    if (is_char) gui_output_buf += (char)value;
    else         gui_output_buf += to_string(value);
}

struct CPU : public DhadCPU {
    int      stack_top = -1;
    uint32_t inst_count = 0;
    string   output = "";
    CPU() {
        dhad_cpu_init(this);
        output_func = gui_output_callback;
...
static void cpu_step(CPU& c){
    gui_output_buf.clear();
    dhad_cpu_step(&c);
    c.output += gui_output_buf;
    c.inst_count++;
    /* Update stack_top to track sp */
    c.stack_top = (c.sp > 0) ? c.sp - 1 : -1;
}
...
private slots:
    ...
    void do_assemble(){
        ...
        memcpy(&nc.memory[PROG_START],a->program,a->prog_size);
        delete a;
    ...
    void do_reset(){
        ...
        memcpy(&nc.memory[PROG_START],&cpu->memory[PROG_START],cpu->prog_size);
        delete cpu;cpu=new CPU(nc);running=false;run_timer->stop();
        btn_run->setText(QString::fromUtf8("تشغيل"));btn_run->setStyleSheet(SHEET_BTN_SUCCESS);
        update_all();
    }
...
private:
    ...
    void update_all(){
        ...
        cycle_gauge->setTarget((int)cpu->cycles);
    ...
    void update_memory(){
        ...
        for(int i=0;i<16&&(start+i)<MEM_SIZE;i++){
            uint16_t addr=start+i;uint8_t val=cpu->memory[addr];
            ...
            QString inst;
            if(cur){
                if(opc==0xF){uint8_t ex=(addr+1<MEM_SIZE)?cpu->memory[addr+1]:0;inst=QString(">> %1").arg(op_name(ex&0xF0));}
    ...
    void update_instructions(){
        ...
        for(int i=0;i<14&&(start+i)<MEM_SIZE;i++){
            ...
            QString inst;
            if(opc==0xF){
                uint8_t ex=(addr+1<MEM_SIZE)?cpu->memory[addr+1]:0;
    ...
    uint16_t fetch16_addr(uint16_t addr){
        uint8_t lo=(addr+1<MEM_SIZE)?cpu->memory[addr+1]:0;
        uint8_t hi=(addr+2<MEM_SIZE)?cpu->memory[addr+2]:0;
        return (hi<<8)|lo;
    }
