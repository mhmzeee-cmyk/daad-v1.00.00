...
void SandboxValidator::checkIncludes(const std::string& source, int& lineNum) {
    // Skip comments (including indented ones like "  // ...")
    {
        size_t firstNonWS = source.find_first_not_of(" \t");
        if (firstNonWS != std::string::npos && source.compare(firstNonWS, 2, "//") == 0) return;
    }
...
void SandboxValidator::checkDangerousPatterns(const std::string& source, int& lineNum) {
    ...
    for (const auto& [pattern, message] : BLOCKED_PATTERNS) {
        ...
        // Use proper regex matching instead of literal string search
        try {
            ...
            // Invalid regex pattern — skip (treat as non-matching)
        }
    }
...
void SandboxValidator::checkRecursionDepth(const std::string& source, int&) {
...
void SandboxValidator::checkMemoryAllocation(const std::string& source, int&) {
