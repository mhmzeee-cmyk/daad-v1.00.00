...
void CodeGenVisitor::visit(SwitchStmtAST& node) {
    ...
    for (const auto& sc : node.cases) {
        if (dynamic_cast<StringExprAST*>(sc.value.get())) {
            hasStringCases = true;
            break;
        }
    }
...
void CodeGenVisitor::visit(StructDeclAST& node) {
    // Write to header (for consistency) and also to source (so main can see it without extra include)
    std::string def = "struct " + sanitizeIdent(node.name) + " {\n";
    for (auto& mem : node.members) {
        std::string cppType = mapType(mem.first);
        def += "    " + cppType + " " + sanitizeIdent(mem.second) + ";\n";
    }
    def += "};\n";
    m_headerStream << def;
    target() << def;
...
void CodeGenVisitor::visit(TryCatchStmtAST& node) {
    emitSourceComment(node.line);
    bool hasCatch = !node.catchBody.empty();
    bool hasFinally = !node.finallyBody.empty();
    if (!hasFinally) {
        target() << "try {\n";
        ...
        if (hasCatch) {
            ...
            if (!node.catchVar.empty()) {
                target() << " (const std::exception& " << sanitizeIdent(node.catchVar) << ")";
            }
            target() << " {\n";
    ...
    // With finally: use RAII guard to guarantee execution on return/exception
    static int finallyCounter = 0;
    int id = finallyCounter++;
    target() << "{\n";
    target() << "auto _daad_finally_" << id << " = [&]() {\n";
    for (auto& stmt : node.finallyBody) {
        stmt->accept(*this);
    }
    target() << "};\n";
    target() << "struct _DaadFinallyGuard" << id << " { decltype(_daad_finally_" << id << ") _fn; ~_DaadFinallyGuard" << id << "() { _fn(); } } _daad_guard_" << id << "{_daad_finally_" << id << "};\n";
    target() << "try {\n";
    for (auto& stmt : node.tryBody) {
        stmt->accept(*this);
    }
...
void CodeGenVisitor::visit(NullExprAST&) {
    m_lastExpr = "nullptr";
}
...
void CodeGenVisitor::visit(ExportStmtAST&) {
