...
void KeywordRegistry::initializeStandardKeywords() {
    ...
    for (const auto& [text, type] : m_keywordToType) {
        ...
        switch (type) {
            ...
            case KeywordType::KwBase:      info.cppEquivalent = ""; break; // metadata فقط — "الأصل" تعني parent/super وليست this، لا يوجد مكافئ مباشر في C++
            case KeywordType::KwAbstract:  info.cppEquivalent = "virtual"; break;

            // 4. الدوال وإدارة الذاكرة
            case KeywordType::KwFunction:  info.cppEquivalent = ""; break; // metadata فقط — "دالة" تعريف دالة وليست نوع إرجاع، Parser يعالجها بشكل خاص
            ...
            case KeywordType::KwInput:       info.cppEquivalent = ""; break; // handled as InputStmt
