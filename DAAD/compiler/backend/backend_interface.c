        ...
        case IR_OP_CALL:
            if (inst->operand_count > 0) {
                ...
                if (strcmp(func_name, "اطبع") == 0) {
                     ...
                     * XMM caller-saved (xmm0-xmm7) are handled architecturally via
                     * movsd spill/reload, alongside integer regs via movq. */
                    PhysicalRegister int_regs[16], float_regs[16];
                    int int_cnt = 0, float_cnt = 0;
                    if (ctx->reg_alloc) {
                        for (int i = 0; i < ctx->reg_alloc->interval_count; i++) {
                            LiveInterval* iv = &ctx->reg_alloc->intervals[i];
                            if (iv->phys_reg == REG_NONE || iv->is_spilled) continue;
                            if (!reg_alloc_is_caller_saved(ctx->reg_alloc, iv->phys_reg)) continue;
                            if (iv->phys_reg < REG_XMM0) {
                                int found = 0;
                                for (int k = 0; k < int_cnt; k++) if (int_regs[k] == iv->phys_reg) { found = 1; break; }
                                if (!found && int_cnt < 16) int_regs[int_cnt++] = iv->phys_reg;
                            } else {
                                int found = 0;
                                for (int k = 0; k < float_cnt; k++) if (float_regs[k] == iv->phys_reg) { found = 1; break; }
                                if (!found && float_cnt < 16) float_regs[float_cnt++] = iv->phys_reg;
                            }
                        }
                    }
                    int save_size = (int_cnt + float_cnt) * 8;
                    if (save_size % 16 != 0) save_size += 8; /* keep 16B ABI alignment */
                    if (save_size > 0)
                        fprintf(out, "  subq $%d, %%rsp\n", save_size);
                    if (ctx->reg_alloc) {
                        int save_idx = 0;
                        for (int k = 0; k < int_cnt; k++) {
                            fprintf(out, "  movq %s, %d(%%rsp)\n",
                                    reg_alloc_phys_name(int_regs[k]), save_idx * 8);
                            save_idx++;
                        }
                        for (int k = 0; k < float_cnt; k++) {
                            fprintf(out, "  movsd %s, %d(%%rsp)\n",
                                    reg_alloc_phys_name(float_regs[k]), save_idx * 8);
                            save_idx++;
                        }
                    }
                    ...
                    if (ctx->reg_alloc) {
                        ...
                        for (int k = 0; k < int_cnt; k++) {
                            fprintf(out, "  movq %d(%%rsp), %s\n",
                                    save_idx * 8, reg_alloc_phys_name(int_regs[k]));
                            save_idx++;
                        }
                        for (int k = 0; k < float_cnt; k++) {
                            fprintf(out, "  movsd %d(%%rsp), %s\n",
                                    save_idx * 8, reg_alloc_phys_name(float_regs[k]));
                            save_idx++;
                        }
                    }
                    if (save_size > 0)
...
int backend_emit_return(IRValue value, FILE* out) {
    ...
    if (value.kind == IR_VALUE_CONSTANT_INT) {
        ...
        fprintf(out, "  movsd .LCf%d(%%rip), %%xmm0\n", float_const_id(value.as.float_val));
