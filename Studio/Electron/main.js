...
function startServer() {
  return new Promise((resolve) => {
    const serverPath = path.join(__dirname, "..", "Dhad-Studio-Full", "server", "src", "index.js");
    serverProcess = spawn(process.execPath, [serverPath], {
      cwd: path.join(__dirname, "..", "Dhad-Studio-Full", "server"),
...
function createMainWindow() {
  ...
  mainWindow.webContents.session.webRequest.onHeadersReceived((details, callback) => {
    callback({
      responseHeaders: {
        ...
        'Content-Security-Policy': ["default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' data:; connect-src 'self' http://localhost:* https:; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'"],
  ...
  mainWindow.loadFile(path.join(__dirname, "..", "Dhad-Studio-Full", "frontend", "frontend-web", "login.html"));
  ...
  const template = [
    ...
    },
    {
      ...
      submenu: [
        ...
        { label: "التوثيق", click: () => shell.openExternal("http://localhost:3000/api-docs") },
      ],
    },
  ];
...
function nav(page) {
  mainWindow?.loadFile(path.join(__dirname, "..", "Dhad-Studio-Full", "frontend", "frontend-web", page));
}
