 ...
 * Also syncs frontend-web mirrors from the canonical Studio/Web source (copy-at-build).
 * Ensures .prisma generated client is in a visible location for ASAR inclusion.
 *
 * DRIFT GUARD: At the end of this script, a diff -qr check verifies that all
 * frontend-web mirrors are in sync with the canonical Studio/Web directory.
 * If drift is detected, a warning is emitted (non-fatal for now).
 */
...
// ── Canonical source: Studio/Web ─────────────────────────────────────────────
// Both mirrors are populated from Web at build time (copy-at-build).
// Do NOT edit the mirrors directly — always edit Studio/Web.
const FRONTEND_MIRRORS = [
  path.join(PROJECT_ROOT, 'Studio', 'Full', 'frontend', 'frontend-web'),   // Full mirror
  path.join(DESKTOP_DIR, 'frontend-web'),                                    // desktop-app mirror (copy target)
];
...
// ── Copy-at-build: Sync frontend-web mirrors from canonical Studio/Web ──────
// This ensures Full/frontend/frontend-web stays in sync with Web without
// requiring manual copies. Both mirrors are populated from the single source.
const FULL_FRONTEND_DST = path.join(PROJECT_ROOT, 'Studio', 'Full', 'frontend', 'frontend-web');
if (fs.existsSync(WEB_SRC)) {
  if (fs.existsSync(FULL_FRONTEND_DST)) {
    fs.rmSync(FULL_FRONTEND_DST, { recursive: true, force: true });
  }
  console.log(`[prepare-build] Copying ${WEB_SRC} -> Studio/Full/frontend/frontend-web`);
  copyRecursive(WEB_SRC, FULL_FRONTEND_DST, []);
} else {
  console.warn(`[prepare-build] Canonical source not found: ${WEB_SRC}`);
}

// ── Drift Guard: Verify all frontend-web mirrors are in sync with Web ────────
// Uses diff -qr to compare the canonical source against each mirror.
// Emits warnings on drift (non-fatal). CI can gate on this in the future.
console.log('[prepare-build] Running drift guard (diff -qr) on frontend mirrors...');
let driftDetected = false;
for (const mirrorPath of FRONTEND_MIRRORS) {
  if (!fs.existsSync(mirrorPath)) {
    console.warn(`[prepare-build] DRIFT: Mirror missing: ${mirrorPath}`);
    driftDetected = true;
    continue;
  }
  try {
    const diffOutput = execSync(
      `diff -qr "${WEB_SRC}" "${mirrorPath}"`,
      { encoding: 'utf8', timeout: 15000, stdio: ['pipe', 'pipe', 'pipe'] }
    );
    if (diffOutput.trim()) {
      console.warn(`[prepare-build] DRIFT detected vs ${mirrorPath}:\n${diffOutput}`);
      driftDetected = true;
    } else {
      console.log(`[prepare-build] OK: ${mirrorPath} is in sync with Web`);
    }
  } catch (err) {
    // diff exits with code 1 when differences exist
    if (err.stdout && err.stdout.trim()) {
      console.warn(`[prepare-build] DRIFT detected vs ${mirrorPath}:\n${err.stdout}`);
      driftDetected = true;
    } else if (err.status !== 0 && err.stderr) {
      console.warn(`[prepare-build] DRIFT check error for ${mirrorPath}: ${err.stderr}`);
    }
  }
}
if (driftDetected) {
  console.warn('[prepare-build] ⚠ DRIFT WARNING: Some frontend mirrors are not in sync with Studio/Web.');
  console.warn('[prepare-build]   This is a non-fatal warning. Edit Studio/Web (canonical) and re-run build.');
  console.warn('[prepare-build]   Future CI will fail on drift detection.');
} else {
  console.log('[prepare-build] ✓ All frontend mirrors are in sync with Studio/Web');
}
