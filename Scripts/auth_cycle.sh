...
# Original: J=/tmp/dhad-phase4-5/server/AC.txt  (hardcoded phase path)
TMP_BASE="${DAAD_TMP:-/tmp/dhad-auth}"
mkdir -p "$TMP_BASE/server" "$TMP_BASE/logs"
J="$TMP_BASE/server/AC.txt"; rm -f $J
pass=0; fail=0
...
R=$(curl -s -c $J -b $J -o "$TMP_BASE/logs/r1.json" -w "%{http_code}" -X POST $B/api/v1/auth/refresh -H "X-CSRF-Token: $CSRF"); chk "first-refresh" $R 200
R=$(curl -s -c $J -b $J -o "$TMP_BASE/logs/r2.json" -w "%{http_code}" -X POST $B/api/v1/auth/refresh -H "X-CSRF-Token: $CSRF"); chk "second-refresh" $R 200
...
OLDJ="$TMP_BASE/server/OLD.txt"; cp $J $OLDJ 2>/dev/null
